...
public final class SceneManager {
    ...
    public SceneManager(ActorManager actorManager, ActorPlaybackController playbackController, SceneCameraService sceneCameras, CameraController cameraController, DirectorController directorController) {
        this.actorManager=actorManager; this.playbackController=playbackController; this.sceneCameras=sceneCameras; this.cameraController=cameraController; this.directorController=directorController;
    }
    ...
    public int play(Player viewer,String sceneName){Scene s=get(sceneName).orElseThrow(()->new IllegalArgumentException("scene not found: "+sceneName));stop(viewer);ensureDuration(s);int started=startActors(viewer,s);ScenePlaybackSession session=new ScenePlaybackSession(s);activeSessions.put(viewer.getUniqueId(),session);renderAt(viewer,session);return started;}
    ...
    private void renderAt(Player viewer,ScenePlaybackSession session,double previousTick){double tick=session.cursor().tick();playbackController.tickScene(viewer,session.scene().actorIds(),tick);boolean directed=directorController.renderAt(viewer,session.scene(),tick);if(!directed&&sceneCameras.camera(session.scene()).isPresent()){var camera=sceneCameras.camera(session.scene()).get();if(!cameraController.active(viewer)||cameraController.currentCamera(viewer)!=camera)cameraController.start(viewer,camera,tick);else cameraController.seek(viewer,tick);}if(Math.abs(tick-previousTick)>1.0e-9d){boolean reverse=session.scene().timeline().reverse();if(session.scene().timeline().loop()){double loopStart=session.scene().timeline().inPoint();double loopEnd=session.scene().timeline().outPoint();if((!reverse&&tick<previousTick)||(reverse&&tick>previousTick))effectPlayback.advanceLoop(viewer,session.scene(),session.effectCursor(),tick,loopStart,loopEnd,reverse);else effectPlayback.advance(viewer,session.scene(),session.effectCursor(),tick,reverse);}else effectPlayback.advance(viewer,session.scene(),session.effectCursor(),tick,reverse);}}
    ...
    public boolean stop(Player viewer){ScenePlaybackSession s=activeSessions.remove(viewer.getUniqueId());if(s==null)return false;for(ActorId id:s.scene().actorIds())playbackController.stop(viewer,id);if(sceneCameras.camera(s.scene()).isPresent()||directorController.active(viewer))cameraController.stop(viewer);return true;}
