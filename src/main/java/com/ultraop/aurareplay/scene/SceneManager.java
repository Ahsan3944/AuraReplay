...
public final class SceneManager {
    ...
    public SceneManager(ActorManager actorManager, ActorPlaybackController playbackController,
        ...
        this.actorManager = actorManager; this.playbackController = playbackController;
        this.sceneCameras = sceneCameras; this.cameraController = cameraController;
    }
    ...
    public void tick(Player viewer){ScenePlaybackSession session=activeSessions.get(viewer.getUniqueId());if(session==null)return;boolean active=session.tick();renderAt(viewer,session);if(!active&&session.finished())stop(viewer);}
    private void renderAt(Player viewer,ScenePlaybackSession session){double tick=session.cursor().tick();playbackController.tickScene(viewer,session.scene().actorIds(),tick);if(cameraController.active(viewer))cameraController.seek(viewer,tick);long effectTick=Math.round(tick);if(Math.abs(tick-effectTick)<0.000001d)for(EffectCue cue:session.scene().effects().at(effectTick))EffectCuePlayer.play(viewer,cue);}
