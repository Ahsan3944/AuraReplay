...
public final class SceneManager {
    ...
    private void renderAt(Player viewer,ScenePlaybackSession session,double previousTick){double tick=session.cursor().tick();playbackController.tickScene(viewer,session.scene().actorIds(),tick);if(cameraController.active(viewer))cameraController.seek(viewer,tick);if(Math.abs(tick-previousTick)>1.0e-9d){boolean reverse=session.scene().timeline().reverse();if(session.scene().timeline().loop()){double loopStart=session.scene().timeline().inPoint();double loopEnd=session.scene().timeline().outPoint();if((!reverse&&tick<previousTick)||(reverse&&tick>previousTick))effectPlayback.advanceLoop(viewer,session.scene(),session.effectCursor(),tick,loopStart,loopEnd,reverse);else effectPlayback.advance(viewer,session.scene(),session.effectCursor(),tick,reverse);}else effectPlayback.advance(viewer,session.scene(),session.effectCursor(),tick,reverse);}}
