...
public final class MotionEvaluator {
    ...
    public Evaluation evaluate(double timelineTick, int frameCount, ActorTransform base, MotionStack stack) {
        ...
        if (frameCount <= 0 || !Double.isFinite(timelineTick))
            return new Evaluation(0.0d, base, false, false);
        ...
        for (MotionLayer layer : stack.layers()) {
            ...
            switch (layer.type()) {
                ...
                case MIRROR_X -> {
                    if (active) { position = mirrorPosition(position, frameCount); px = -px; yaw = normalize(yaw + 180.0f); }
                }
                case MIRROR_Z -> {
                    if (active) { position = mirrorPosition(position, frameCount); pz = -pz; yaw = normalize(180.0f - yaw); }
                }
                ...
                case POSITION_OFFSET, PATH_SHIFT -> {
                    if (active) {
                        px += layer.x() * blend;
                        py += layer.y() * blend;
                        pz += layer.z() * blend;
                ...
                case ROTATION_OFFSET -> {
                    if (active) {
                        yaw = normalize((float) (yaw + layer.yaw() * blend));
                        pitch = normalize((float) (pitch + layer.pitch() * blend));
                        roll = normalize((float) (roll + layer.roll() * blend));
                    }
                }
    ...
    private static double mirrorPosition(double position, int frameCount) {
        return frameCount - 1.0d - position;
    }
