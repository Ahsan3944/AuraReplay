...
public final class DirectorCaptureSession {
    ...
    public DirectorCaptureSession(DirectorExportSpec spec, DirectorCaptureSink sink) {
    ...
    public void accept(DirectorFrame frame) {
        ...
        if (Math.abs(frame.sceneTick() - expectedTick) > 1.0e-9)
            throw new IllegalArgumentException("frame " + nextFrame + " has wrong scene tick: " + frame.sceneTick());
        try {
            sink.accept(frame);
            nextFrame++;
            if (nextFrame >= spec.frameCount()) complete();
        } catch (Throwable ex) {
            fail(ex);
            throw ex;
        }
    ...
    public void cancel() {
        ...
        try { sink.cancel(); } finally { state = State.CANCELLED; }
    }
    ...
    public void fail(Throwable error) {
        ...
        failure = error;
        try { sink.fail(error); } finally { state = State.FAILED; }
    }
}
