...
public final class ActorPlayback {
    ...
    private boolean suppressNextActionEmission;
    ...
    public ActorPlayback(ActorDefinition actor, ActorPlaybackSource source) {
        this.actor = Objects.requireNonNull(actor); this.source = Objects.requireNonNull(source);
        this.actionPlayback = new ActorActionPlayback(actor.actionTimeline()); reset();
    }
    ...
    public List<ActorActionEvent> tickActions() { tick(); return actionsAt(cursor.position()); }
    public void tick() {
        elapsedTicks++;
    ...
    public void setScenePosition(double sceneTick) {
        ...
        boolean continuous = delta > 0.0d && Math.abs(delta - actor.playbackSpeed()) <= 1.0e-9d;
        if (continuous) return;
        seekActionCursorSilently(current);
        suppressNextActionEmission = true;
    }
    public List<ActorActionEvent> actionsAt(double position) {
        ...
        if (reverse != actionDirectionReverse) resetActionCursorForDirection(position, reverse);
        ...
        } else resetActionCursorForDirection(position, reverse);
        if (suppressNextActionEmission) {
            positionActionCursorSilently(tick, reverse);
            lastActionPosition = position; actionDirectionReverse = reverse; suppressNextActionEmission = false;
            return List.of();
        }
        List<ActorActionEvent> events = reverse ? actionPlayback.advanceReverse(tick) : actionPlayback.advance(tick);
        lastActionPosition = position; actionDirectionReverse = reverse; return events;
    }
    public void reset() {
        elapsedTicks = 0L; actionPlayback.reset(); lastActionPosition = Double.NaN;
        actionDirectionReverse = actor.reverse(); suppressNextActionEmission = false;
        if (actor.reverse() && playbackDuration() > 0.0d) {
            cursor.seek(playbackDuration() - 1.0d); actionPlayback.seek((long) Math.ceil(playbackDuration()));
        } else cursor.reset();
    }
    private void seekActionCursorSilently(double position) {
        long tick = Math.max(0L, (long) Math.floor(position)); positionActionCursorSilently(tick, actor.reverse());
        lastActionPosition = position; actionDirectionReverse = actor.reverse();
    }
    private void positionActionCursorSilently(long tick, boolean reverse) {
        actionPlayback.seek(reverse ? Math.max(0L, tick + 1L) : tick);
    }
    private void resetActionCursorForDirection(double position, boolean reverse) {
        long tick = Math.max(0L, (long) Math.floor(position));
        if (reverse) positionActionCursorSilently(tick, true); else actionPlayback.reset();
        lastActionPosition = Double.NaN; actionDirectionReverse = reverse;
    }
    ...
    public ActorSample sampleAt(double position) {
        int count = source.frameCount(); if (count <= 0 || !Double.isFinite(position)) return null;
        MotionEvaluator.Evaluation evaluation = motionEvaluator.evaluate(position, count, actor.transform(), actor.motionStack());
        double bounded = evaluation.sourcePosition(); int lowerIndex = (int) Math.floor(bounded); int upperIndex = Math.min(count - 1, lowerIndex + 1);
        double alpha = bounded - lowerIndex; TickSnapshot lowerFrame = frame(lowerIndex); TickSnapshot upperFrame = frame(upperIndex);
        if (lowerFrame == null || upperFrame == null) return null;
        EntitySnapshot lower = findSource(lowerFrame), upper = findSource(upperFrame);
        if (lower == null) lower = upper; if (upper == null) upper = lower; if (lower == null) return null;
        EntitySnapshot sampled = interpolate(lower, upper, alpha); ActorTransform base = evaluation.transform();
        ActorTransform transformed = new ActorTransform(sampled.x() + base.x(), sampled.y() + base.y(), sampled.z() + base.z(),
                normalizeDegrees(sampled.yaw() + base.yaw()), normalizeDegrees(sampled.pitch() + base.pitch()), base.roll(), base.scale());
    ...
    private TickSnapshot frame(int index) { try { return source.frame(index); } catch (IOException | RuntimeException ignored) { return null; } }
    private EntitySnapshot findSource(TickSnapshot frame) {
        UUID sourceUuid = actor.sourceEntityUuid(); Integer sourceId = actor.sourceEntityId();
    ...
    private EntitySnapshot interpolate(EntitySnapshot a, EntitySnapshot b, double alpha) {
        if (a == b || alpha <= 0.0d) return a; if (alpha >= 1.0d) return b;
        return new EntitySnapshot(a.entityId(), a.uuid(), a.type(), lerp(a.x(), b.x(), alpha), lerp(a.y(), b.y(), alpha), lerp(a.z(), b.z(), alpha),
                interpolateAngle(a.yaw(), b.yaw(), alpha), (float) lerp(a.pitch(), b.pitch(), alpha), lerp(a.velocityX(), b.velocityX(), alpha),
                lerp(a.velocityY(), b.velocityY(), alpha), lerp(a.velocityZ(), b.velocityZ(), alpha), alpha < 0.5d ? a.flags() : b.flags(),
                alpha < 0.5d ? a.equipment() : b.equipment(), alpha < 0.5d ? a.identity() : b.identity(), a.exists() && b.exists());
    }
    ...
    private static final class MemoryActorPlaybackSource implements ActorPlaybackSource {
        private final List<TickSnapshot> frames; private MemoryActorPlaybackSource(List<TickSnapshot> frames) { this.frames = List.copyOf(frames); }
